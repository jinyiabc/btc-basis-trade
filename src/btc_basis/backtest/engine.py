...
Refactored from btc_basis_backtest.py
"""
...
class Backtester:
    ...
    def __init__(self, config: TradeConfig = None):
        ...
        self.analyzer = BasisTradeAnalyzer(self.config)
        ...
        for data_point in historical_data:
            ...
            if current_trade:
                ...
                if should_exit:
                    ...
                    funding_cost = (
                        (self.config.funding_cost_annual / 365)
                        * holding_days_actual
...
"""
        for i, trade in enumerate(result.trades, 1):
            status_symbol = (
                "[+]" if trade.realized_pnl and trade.realized_pnl > 0 else "[-]"
            )
            report += f"{i}. {status_symbol} {trade.entry_date.strftime('%Y-%m-%d')}"
            if trade.exit_date:
                report += (
                    f" -> {trade.exit_date.strftime('%Y-%m-%d')} ({trade.holding_days}d)"
                )
